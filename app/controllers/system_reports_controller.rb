  ...
  private
    ...
    def create_query(report_type)
      if report_type == 'totals_programs'
        sql = "SELECT (SELECT programs.title FROM programs WHERE res.program_id = programs.id) AS program,
      ...
      if report_type == 'vehicle_reports_all'
        ...
        FROM students
        JOIN reservation_passengers ON reservation_passengers.student_id = students.id AND reservation_passengers.reservation_id = vehicle_reports.reservation_id) AS passengers,

...
class Reservation < ApplicationRecord
  ...
  def added_people
    number = all_passengers_count + (self.driver.present? ? 1 : 0) + (self.driver_manager.present? ? 1 : 0)
  ...
  def all_passengers_count
    (self.passengers + self.passengers_managers).size
  end
  ...
  def check_number_of_people_on_trip
    ...
    if self.number_of_people_on_trip_changed?
      if self.number_of_people_on_trip_change[0] > self.number_of_people_on_trip_change[1] && self.number_of_people_on_trip_change[1] < self.added_people
        errors.add(:number_of_people_on_trip, "can't be smaller than the number of people added to the reservation; remove passengers before updating the number.")
      end
    end
  end

  ...
  def get_car_day_reservations_hash(day, car, unit_id)
    ...
    if defined?(@week_calendar_reservations) && @week_calendar_reservations.present?
      ...
      if car.present?
        ...
        car_day_reserv = Reservation.where(program: Program.where(unit_id: unit_id), car_id: nil).where(
          "start_time BETWEEN ? AND ? OR end_time BETWEEN ? AND ? OR (start_time < ? AND end_time > ?)",
          day.beginning_of_day, day.end_of_day, day.beginning_of_day, day.end_of_day, day.beginning_of_day, day.end_of_day
        )  
      end
    end
    car_cells = {}

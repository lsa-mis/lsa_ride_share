...
RSpec.describe Cars::NotesController, type: :request do
  ...
  context 'with admin role' do
    ...
    it 'creates a note and redirects to car page for html requests' do
      expect do
        post car_notes_path(car), params: {
          note: {
            body: 'Admin note for car'
          }
        }
      ...
      expect(created_note.noteable).to eq(car)
      expect(created_note.user_id).to eq(admin_user.id)
      expect(created_note.body.to_plain_text).to include('Admin note for car')
    end

    it 'creates a note and responds with turbo stream' do
      expect do
        post car_notes_path(car, format: :turbo_stream), params: {
          note: {
            body: 'Turbo note for car'
          }
        }
      end.to change(Note, :count).by(1)
    ...
    it 'does not create a note when body is blank and redirects to car page' do
      expect do
        post car_notes_path(car), params: {
          note: {
            body: ''

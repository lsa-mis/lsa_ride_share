...
RSpec.describe Sites::NotesController, type: :request do
  ...
  context 'with admin role' do
    ...
    it 'creates a note and redirects to site page for html requests' do
      expect do
        post site_notes_path(site), params: {
          note: {
            body: 'Admin note for site'
          }
        }
      ...
      expect(created_note.noteable).to eq(site)
      expect(created_note.user_id).to eq(admin_user.id)
      expect(created_note.body.to_plain_text).to include('Admin note for site')
    end

    it 'creates a note and responds with turbo stream' do
      expect do
        post site_notes_path(site, format: :turbo_stream), params: {
          note: {
            body: 'Turbo note for site'
          }
        }
      end.to change(Note, :count).by(1)
    ...
    it 'does not create a note when body is blank and redirects to site page' do
      expect do
        post site_notes_path(site), params: {
          note: {
            body: ''

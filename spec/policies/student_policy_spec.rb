...
RSpec.describe StudentPolicy, type: :policy do
  ...
  context 'with manager role who is a program manager' do
    ...
    let(:program_with_student_and_manager) { FactoryBot.create(:program_with_student_and_manager, instructor: manager) }
    subject { described_class.new({ user: program_manager, params: {program_id: program_with_student.id }, role: "manager" }, program_with_student_and_manager.students) }
  ...
  #   subject { described_class.new({ user: user, role: "none" }, program_with_student.students) }

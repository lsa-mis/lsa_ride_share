...
RSpec.describe ProgramPolicy, type: :policy do
  ...
  context 'with super_admin role' do
    ...
    it { is_expected.to permit_only_actions(%i[is_super_admin user_in_access_group index show create new update edit duplicate get_programs_list get_students_list get_sites_list get_instructor_id]) }
  end
  ...
  context 'with admin role' do
    ...
    it { is_expected.to permit_only_actions(%i[is_admin user_in_access_group index show create new update edit duplicate get_programs_list get_students_list get_sites_list get_instructor_id]) }
  end

  context 'with manager role' do
    subject { described_class.new({ user: user, role: "manager", params: {id: program.id} }, program) }

    it { is_expected.to forbid_actions(%i[is_super_admin is_student is_admin create new get_instructor_id]) }
  ...
  context 'with student role' do
    ...
    it { is_expected.to forbid_actions(%i[is_super_admin is_admin is_manager index show create new update edit duplicate get_programs_list get_students_list get_sites_list get_instructor_id]) }
